...
var (
	...
	scriptNavigationPattern = regexp.MustCompile(`(?is)(?:(?:window\.)?location(?:\.href)?\s*=|location\.replace\()\s*["']?([^"'<>\s)]+)`)
)
...
func htmlNavigation(page *page) (*url.URL, bool) {
	...
	if rawDestination == "" {
		walkHTML(document, func(node *html.Node) bool {
			...
			match := scriptNavigationPattern.FindStringSubmatch(htmlText(node))
			if len(match) == 2 {
				rawDestination = match[1]
				return true
			}
