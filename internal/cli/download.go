...
func downloadCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path, err := convertToMarkdown(cmd, output, name, body)
			if err != nil {
				return err
			}
...
func convertToMarkdown(cmd *cobra.Command, directory, name string, body io.Reader) (string, error) {
	...
	process := exec.CommandContext(cmd.Context(), "docling", "convert", pdfPath, "--to", "md", "--output", directory)
